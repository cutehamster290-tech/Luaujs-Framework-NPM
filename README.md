# Luaujs
An Open-Source Framework that transforms Luau's code in HTML code.
...
## Note
Luaujs comes with some commands and the framework, anything you will type will be similar to this: `luaujs command_name arguments`
...
- `element.appendChild(element: table)`: adds another HTML element into the element, updating some properties of each other parent, childs and also closetag

- `Instance.Result(element: table, Fullhtml: string?)`: prints every single item inside that element and the element itself, so you will see the div with inside the h1 with inside the label with inside the h2. write anything with a Truthy value in the arg2 to show the entire html code (which i suggest).
